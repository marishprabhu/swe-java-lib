...
//  "                    G       (36 Gauquelin sectors)\n"+
  ...
  int randomCount = 0;
    ...
    public Transits(ISwissEph sw) {
        ...
        this.err = new SwissErr();
        this.sw = sw;
    }


    public SwissErr getSwissErr() {
        return err;
    }


    public SwissOut getSwissOut() {
        return out;
    }
...
err.println(np);
    ...
    if (a.convert) {
      ...
      out.println(printFormatted(a,null));
      return 0;
    }
	...
	a.duplicateTransitPoints = savedDuplicateTransitPoints;
      ...
      if (Double.isNaN(tr.jdET) || tr.jdET == Double.MAX_VALUE || tr.jdET == -Double.MAX_VALUE) {
        if (isFirstCalculation) {
          tr.pl1 = a.pl1;
          tr.pl2 = a.pl2;
          err.println(getPlanetnameString(a, tr) + (a.mpp1 ? " etc.: " : "") + " no transit");
        }
        break;
      }
...
//		}
          ...
          if ((swe.getType() & SwissephException.BEYOND_USER_TIME_LIMIT) != 0) {
            ...
            err.println("ERROR: " + swe.getMessage());
          }
        } // try ... catch ...
        if (a.v.back) {
          if (jd > tr.jdET) {
...
//...
  ...
  String verifyPlanet(TransitArguments a, TransitResult tr, int which, int cnt, boolean diff) {
    ...
    if (diff) {
      if (a.pls1 != null && a.pls2 != null) {	// diff between two planets
        ...
        ret = sw.swe_houses_ex(a.sde1.getJulDay() - SweDate.getDeltaT(a.sde1.getJulDay()), house_flags, a.house_lat, a.house_lon, a.hsys, cusps, ascmc);
  ...
  String verifyHouseobject(TransitArguments a, TransitResult tr, int which, int cnt) {
    ...
    int ret = sw.swe_houses_ex(a.sde1.getJulDay() - SweDate.getDeltaT(a.sde1.getJulDay()), house_flags, a.house_lat, a.house_lon, a.hsys, cusps, ascmc);
...
err.println("Warning: Locale '" + locale + "' not found, using default locale '" + defLocale + "'");
err.println("Use option -locales to list all available Locales.");
    return defLocale;
  }
    ...
    for (int i = 0; i < argv.length; i++) {
      if (argv[i].equals("-hdate")) {
        ...
        return(null);
                 ...
                 argv[i].equals("-?") ||
                 argv[i].equals("/?")) {
        out.println(infocmd0);
        out.println(infocmd1);
        if (infoexamples == null) {
          initHelpTexts();
        }
        out.println(infocmd2);
        out.println(infoexamples);
        sw.swe_close();
        return(null);
  ...
  TransitArguments initArgs(TransitArguments a) {
    ...
    if (a.convert) {
      ...
      for(int r=0;r<invalids.length();r++) {
        ...
        while (idx >= 0) {
          out.println("Info: ignoring output format character '" + ch + "' on date conversions.");
      ...
      if (a.enddate != null || !"".equals(a.sEndhour)) {
        err.println("Warning: Ignoring a second date on -cv option.");
      }
      if (a.back) {
        err.println("Warning: Ignoring option -r with -cv.");
      }
      if (a.calcSpeed) {
        err.println("Warning: Ignoring option -s with -cv.");
      }
      if (a.sidmode != -1) {
        err.println("Warning: Ignoring option -sid... with -cv.");
      }
      if (a.pl1 != -2 || a.pl2 != -2) {
        err.println("Warning: Ignoring options -p... / -P... options with -cv.");
      }
      if (a.countIsSet) {
        err.println("Warning: Ignoring options -n... / -N... with -cv.");
      }
// ...
      return a;
    }
...
// System.err.println("SD: " + a.sde1); java Transits -f+pP -dloc -b1.1.0 -ut -house88,0,E -lon0 -ohouse1 will return ET date(?) if a.sde1.toString() isn't called before???
    ...
    if (a.pls2 == null && a.objects2 == null && a.outputFormat.indexOf('P') >= 0) {
      out.println("Info: ignoring output format character 'P' on non-relative transits.");
    }
...
//System.err.println("SD 1: " + a.sde1);
  ...
  void writeCmdLine(String[] par, boolean withHeader) {
    if (withHeader) {
      for (int i = 0; i < par.length; i++) { out.print(par[i]+" "); }
      out.println();
    }
  }
...
// Should state the ayanamsha system here!
      if (a.enddate == null) {
         out.print(":         "+(a.mpp1 || a.mpo1?" ":"")+
                                     doubleToDMS(sw.swe_get_ayanamsa(a.tjde1)));
      } else {
         out.println("\n on starting date: "+(a.mpp1 || a.mpo1?" ":"")+
                                     doubleToDMS(sw.swe_get_ayanamsa(a.tjde1)));
         out.println(" on end date:      "+(a.mpp1 || a.mpo1?" ":"")+
                                    doubleToDMS(sw.swe_get_ayanamsa(a.tjde2)));
      }
    }
    out.println();
    if (a.isLt) {
      out.println("Using localtime:   UTC+"+nnof.format(tzOffset*24) + "h");
    }
    out.println("Starting date:     "+(a.mpp1 || a.mpo1?" ":"")+jdToDate(a.sde1, a.isUt, a.isLt, 0)+(a.isLt?" LT":(a.isUT?" UT":" ET")));
    if (a.enddate!=null) {
      out.println("End date:          "+(a.mpp1 || a.mpo1?" ":"")+jdToDate(a.sde2, a.isUT, a.isLT, 0)+(a.isLT?" LT":(a.isUT?" UT":" ET")));
    }

    if (!a.yogaTransit && !a.partileStart && !a.partileEnd) {
      if (a.pls1 != null) {
        out.println("Transiting planet" + (a.mpp1?"s":"") + ": " +
        ...
        out.println("Transiting object" + (a.mpo1?"s":"") + ": " +
    ...
    if (a.calcSpeed) {	// no house object transits possible
      ...
      if (a.pls2 != null) {
        ...
        out.println(limitLineLength(line,
                           70,
                           "                   "+(a.mpp1 || a.mpo1?" ":"")));
      } else {
      ...
      if ((a.iflag&SweConst.SEFLG_TOPOCTR)!=0) {
        out.println("Topographic pos.:  "+(a.mpp1 || a.mpo1?" ":"") +
      ...
      if ((a.iflag&SweConst.SEFLG_TOPOCTR)!=0) {
        if (a.objects1 != null) {
          out.println("Topographic pos.:  " + (a.mpp1?" ":"") +
              doubleToDMS(SMath.abs(a.house_lon)) + (a.house_lon<0?"W":"E") + "/" +
              doubleToDMS(SMath.abs(a.house_lat)) + (a.house_lat<0?"S":"N") + "/" +
              nnof.format(a.top_elev) + "m");
        } else {
